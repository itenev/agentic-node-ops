...
def execute_command(cmd: str, timeout: int = 30, shell_required: bool = False) -> dict[str, Any]:
...
def execute_action(action: RunbookAction) -> dict[str, Any]:
    ...
    if action.requires_explicit_unlock:
        log.warning("Blocked execution of privileged action '%s': requires explicit unlock", action.id)
        return {
            "id": action.id,
            "cmd": action.cmd,
            "success": False,
    ...
    res = execute_command(action.cmd, timeout=timeout_sec, shell_required=action.shell_required)
    return {
        "id": action.id,
        "cmd": action.cmd,
