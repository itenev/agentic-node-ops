...
def check_timeout_escalation(action_id: str, incident_id: str, db: DB) -> None:
    ...
    if timeouts >= 2:
        # Stop proposing this action; send a single escalation message
        notify_escalation(
            message=(
...
def group_pending_proposals(host: str, db: DB) -> list[ProposalGroup]:
    ...
    # Bundle into one grouped message with per-action approve/skip buttons
...
Note: `action_proposals` is required by the approval fatigue logic in §5 — `db.get_last_proposal()`, `db.count_timeouts()`, and `db.mark_action_suppressed()` all read/write this table.
...
The `depends_on: webhook-receiver` on hermes-agent ensures the receiver container is created and started first so the shared `alerts.jsonl` file path is initialized. At runtime, if the receiver crashes and restarts, the agent continues reading from the jsonl file normally.
